...
public class TestDao {
	...
	public boolean saveData(String filename){
		boolean back = true;
		try {
			conn = JDBCUtil.getTestDbConn();//关联用户对应的数据文件
			String addr = "INSERT INTO Tb_User (FName) VALUES (?)";
			sta = conn.prepareStatement(addr);
	...
	//保存用户的数据
	public boolean saveDataForApp(WebResponse webResponse,String filename){
		boolean back = true;
		try {
			conn = JDBCUtil.getUserDataConn(filename);//关联用户对应的数据文件
			//删除所有文件
			sta = conn.prepareStatement("DELETE  FROM Tb_BuyBean");
			sta.execute();
			sta = conn.prepareStatement("DELETE  FROM Tb_BuyAtBean");
			sta.execute();
			sta = conn.prepareStatement("DELETE  FROM Tb_AddrBean");
			sta.execute();
			sta = conn.prepareStatement("DELETE  FROM Tb_NoteBean");
			sta.execute();
			Lg.e("数据删除成功....");
			String addr = "INSERT INTO Tb_AddrBean (FID, FName,FMapID,FUseNum,FIsCloud,FCreateData) VALUES (?,?,?,?,?,?)";
			String buyat = "INSERT INTO Tb_BuyAtBean (FID, FName,FNum,FAddrID,FAddrName,FBuyID,FBuyName,FCreateData,FIsCloud,FUseNum,FMapID) VALUES (?,?,?,?,?,?,?,?,?,?,?)";
			String buy = "INSERT INTO Tb_BuyBean (FID, FName,FCreateData,FIsCloud,FUseNum) VALUES (?,?,?,?,?)";
			String note = "INSERT INTO Tb_NoteBean (FID,NBuyName,NAddrName,Ntime,NCreateTime) VALUES (?,?,?,?,?)";
			sta = conn.prepareStatement(addr);
			for (int i = 0; i < webResponse.addrBeans.size(); i++) {
				sta.setString(1,webResponse.addrBeans.get(i).FID);
				sta.setString(2,webResponse.addrBeans.get(i).FName);
				sta.setString(3,webResponse.addrBeans.get(i).FMapID);
				sta.setString(4,webResponse.addrBeans.get(i).FUseNum);
				sta.setString(5,webResponse.addrBeans.get(i).FIsCloud);
				sta.setString(6,webResponse.addrBeans.get(i).FCreateData);
				sta.executeUpdate();
			}
			sta = conn.prepareStatement(buyat);
			for (int i = 0; i < webResponse.buyAtBeans.size(); i++) {
				sta.setString(1,webResponse.buyAtBeans.get(i).FID);
				sta.setString(2,webResponse.buyAtBeans.get(i).FName);
				sta.setString(3,webResponse.buyAtBeans.get(i).FNum);
				sta.setString(4,webResponse.buyAtBeans.get(i).FAddrID);
				sta.setString(5,webResponse.buyAtBeans.get(i).FAddrName);
				sta.setString(6,webResponse.buyAtBeans.get(i).FBuyID);
				sta.setString(7,webResponse.buyAtBeans.get(i).FBuyName);
				sta.setString(8,webResponse.buyAtBeans.get(i).FCreateData);
				sta.setString(9,webResponse.buyAtBeans.get(i).FIsCloud);
				sta.setString(10,webResponse.buyAtBeans.get(i).FUseNum);
				sta.setString(11,webResponse.buyAtBeans.get(i).FMapID);
				sta.executeUpdate();
			}
			sta = conn.prepareStatement(buy);
			for (int i = 0; i < webResponse.buyBeans.size(); i++) {
				sta.setString(1,webResponse.buyBeans.get(i).FID);
				sta.setString(2,webResponse.buyBeans.get(i).FName);
				sta.setString(3,webResponse.buyBeans.get(i).FCreateData);
				sta.setString(4,webResponse.buyBeans.get(i).FIsCloud);
				sta.setString(5,webResponse.buyBeans.get(i).FUseNum);
				sta.executeUpdate();
			}
			sta = conn.prepareStatement(note);
			for (int i = 0; i < webResponse.noteBeans.size(); i++) {
				sta.setString(1,webResponse.noteBeans.get(i).FID);
				sta.setString(2,webResponse.noteBeans.get(i).NBuyName);
				sta.setString(3,webResponse.noteBeans.get(i).NAddrName);
				sta.setString(4,webResponse.noteBeans.get(i).Ntime);
				sta.setString(5,webResponse.noteBeans.get(i).NCreateTime);
				sta.executeUpdate();
			}
			Lg.e("数据新增完成");
		} catch (ClassNotFoundException e) {
	...
	//保存用户的数据
	public ArrayList<NoteBean> getNoteDataForApp(String filename){
		Lg.e("getNoteDataForApp",filename);
		ArrayList<NoteBean> noteBeans = new ArrayList<>();
		try {
			conn = JDBCUtil.getUserDataConn(filename);//关联用户对应的数据文件
			String note = "SELECT FID,NBuyName,NAddrName,Ntime,NCreateTime FROM Tb_NoteBean";
			sta = conn.prepareStatement(note);
			rs = sta.executeQuery();
			while (rs.next()) {
				NoteBean bean = new NoteBean();
				bean.FID = rs.getString("FID");
				bean.NBuyName = rs.getString("NBuyName");
				bean.NAddrName = rs.getString("NAddrName");
				bean.Ntime = rs.getString("Ntime");
				bean.NCreateTime = rs.getString("NCreateTime");
				noteBeans.add(bean);
			}
			Lg.e("数据获取完成");
		} catch (ClassNotFoundException e) {
	...
	public ArrayList<BuyAtBean> getBuyAtDataForApp(String filename,String buyname){
		Lg.e("查询",filename+"___"+buyname);
		ArrayList<BuyAtBean> noteBeans = new ArrayList<>();
		try {
			conn = JDBCUtil.getUserDataConn(filename);//关联用户对应的数据文件
//			String note = "SELECT FID,NBuyName,NAddrName,Ntime,NCreateTime FROM Tb_NoteBean";
			String buyat = "SELECT FID, FName,FNum,FAddrID,FAddrName,FBuyID,FBuyName,FCreateData,FIsCloud,FUseNum,FMapID FROM Tb_BuyAtBean WHERE FBuyName ='"+buyname+"'";
			sta = conn.prepareStatement(buyat);
			rs = sta.executeQuery();
			while (rs.next()) {
				BuyAtBean bean = new BuyAtBean();
				bean.FID = rs.getString("FID");
				bean.FName = rs.getString("FName");
				bean.FNum = rs.getString("FNum");
				bean.FAddrID = rs.getString("FAddrID");
				bean.FAddrName = rs.getString("FAddrName");
				bean.FBuyID = rs.getString("FBuyID");
				bean.FBuyName = rs.getString("FBuyName");
				bean.FCreateData = rs.getString("FCreateData");
				bean.FIsCloud = rs.getString("FIsCloud");
				bean.FUseNum = rs.getString("FUseNum");
	...
	public String getBuyAtDataSize(String filename,String buyname){
		...
		try {
			conn = JDBCUtil.getUserDataConn(filename);//关联用户对应的数据文件
//			String note = "SELECT FID,NBuyName,NAddrName,Ntime,NCreateTime FROM Tb_NoteBean";
			String buyat = "SELECT COUNT(*) AS 数量 FROM Tb_BuyAtBean WHERE FBuyName ='"+buyname+"'";
			sta = conn.prepareStatement(buyat);
			rs = sta.executeQuery();
			while (rs.next()) {
				noteBeans = rs.getString("数量");
			}
			Lg.e("数据获取完成");
		} catch (ClassNotFoundException e) {
			e.printStackTrace();
	...
	public boolean getBuyAtDataDelForApp(String filename,String buyname,String delAddr){
		...
		try {
			conn = JDBCUtil.getUserDataConn(filename);//关联用户对应的数据文件
//			String note = "SELECT FID,NBuyName,NAddrName,Ntime,NCreateTime FROM Tb_NoteBean";
			String buyat = "DELETE  FROM Tb_BuyAtBean WHERE FBuyName ='"+buyname+"' AND FCreateData = '"+delAddr+"'";
			sta = conn.prepareStatement(buyat);
			isOk = sta.execute();
		} catch (ClassNotFoundException e) {
			e.printStackTrace();
	...
	public boolean getAllDataForApp(WebResponse webResponse,String filename){
		...
		webResponse.buyBeans = new ArrayList<>();
		webResponse.noteBeans = new ArrayList<>();
		boolean back = true;
		try {
			conn = JDBCUtil.getUserDataConn(filename);//关联用户对应的数据文件
			String addr = "SELECT FID, FName,FMapID,FUseNum,FIsCloud,FCreateData FROM Tb_AddrBean";
			String buyat = "SELECT FID, FName,FNum,FAddrID,FAddrName,FBuyID,FBuyName,FCreateData,FIsCloud,FUseNum,FMapID FROM Tb_BuyAtBean";
			String buy = "SELECT FID, FName,FCreateData,FIsCloud,FUseNum FROM Tb_BuyBean";
			String note = "SELECT FID,NBuyName,NAddrName,Ntime,NCreateTime FROM Tb_NoteBean";
			sta = conn.prepareStatement(addr);
			rs = sta.executeQuery();
			while (rs.next()) {
				...
				bean.FIsCloud = rs.getString("FIsCloud");
				bean.FCreateData = rs.getString("FCreateData");
				webResponse.addrBeans.add(bean);
			}
			sta = conn.prepareStatement(buyat);
			rs = sta.executeQuery();
			while (rs.next()) {
				...
				bean.FUseNum = rs.getString("FUseNum");
				bean.FMapID = rs.getString("FMapID");
				webResponse.buyAtBeans.add(bean);
			}
			sta = conn.prepareStatement(buy);
			rs = sta.executeQuery();
			while (rs.next()) {
			...
			while (rs.next()) {
				...
				bean.Ntime = rs.getString("Ntime");
				bean.NCreateTime = rs.getString("NCreateTime");
				webResponse.noteBeans.add(bean);
			}
			Lg.e("数据获取完成");
		} catch (ClassNotFoundException e) {
			back = false;
	...
	public String getDataCountForApp(String filename){
		...
		try {
			conn = JDBCUtil.getUserDataConn(filename);//关联用户对应的数据文件
			//删除所有文件
			sta = conn.prepareStatement("SELECT COUNT(*) AS 数量 FROM Tb_BuyBean");
			rs = sta.executeQuery();
			while (rs.next()) {
				num += MathUtil.toD(rs.getString("数量"));
			}
			sta = conn.prepareStatement("SELECT COUNT(*) AS 数量 FROM Tb_BuyAtBean ");
			rs = sta.executeQuery();
			while (rs.next()) {
				num += MathUtil.toD(rs.getString("数量"));
			}
			sta = conn.prepareStatement("SELECT COUNT(*) AS 数量 FROM Tb_AddrBean ");
			rs = sta.executeQuery();
			while (rs.next()) {
				num += MathUtil.toD(rs.getString("数量"));
			}
			sta = conn.prepareStatement("SELECT COUNT(*) AS 数量 FROM Tb_NoteBean ");
			rs = sta.executeQuery();
			while (rs.next()) {
				num += MathUtil.toD(rs.getString("数量"));
			}
